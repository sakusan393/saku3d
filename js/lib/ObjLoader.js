...
(function() {
	...
	ObjLoader.load = function(srcFiles, cb) {
		var callback = function() {
			...
			if(fileCount == 0) {
				// 全ファイルがロードされたら初期化
				initialize(objSrc,mtlSrc,cb);
			}
		};
		var objSrc = srcFiles.obj;
		var mtlSrc = srcFiles.mtl;
		_loadFile(srcFiles.obj, objSrc, callback);
		_loadFile(srcFiles.mtl, mtlSrc, callback);
	};
	ObjLoader.files = {};
	var _loadFile = function(url, name, callback) {
		fileCount++;
	...
	var initialize = function (objSrc,mtlSrc,cb) {
		var obj = ObjParser.objParse(objSrc);
		var mtl = ObjParser.mtlParse(mtlSrc);
	...
	// MTLファイルのパース
	ObjParser.mtlParse = function(mtlSrc) {
		var text = ObjLoader.files[mtlSrc];
		var mtl = {};
        ...
        case "d":
				...
				case "map_Kd":
					// テクスチャの保存
					current.texture = words[1].replace("\n","").replace("\r","");
					break;
			}
		}
		// ファイルの最後のmtl情報を保存する
	...
	// OBJファイルのパース
	ObjParser.objParse = function(objSrc) {
		var text = ObjLoader.files[objSrc]
		var vertices = []; // 頂点座標
		var normals = []; // 法線ベクトル
    ...
    if(obj.texcoords.length) texcoords = new Float32Array(numTriangles * 6);
		...
		var saveMtlInfo = function() {
			if(!mtl) {
				...
				var dir = mtl.mtlSrc.split("/")[0]
				if(textureName) {
					ImageLoader.load([String(dir+"/"+textureName)],function(){
